...
void handle_request(const std::string& request, ip::tcp::socket& frontend_socket, ip::tcp::socket& fpga_socket)
{
    try
    {
        ...
        // Go through the entire image pixel-by-pixel, and multiply by the flattened nine surrounding pixels for each to perform convolution using multiplication. The input
        // matrices should be 1x9 and 9xn^2 respectively, for an output matrix of nxn.
        ...
        // This could go within the if statement below, but it's useful to print out_fpga_payload for debugging purposes even without FPGA in the loop
        size_t out_fpga_payload_size = (FPGA_PAYLOAD_DIMS_SIZE + 9 + image_data_matrix.get_num_rows() * image_data_matrix.get_num_cols());
        ...
        if (FPGA_IN_LOOP)
        {
            ...
            int pos = 0;
            for (int row = 0; row < result_matrix.get_num_rows(); row++)
            {
                for (int col = 0; col < result_matrix.get_num_cols(); col++)
                {
                    result_matrix[row][col] = ADC_CONVERSION_FACTOR * in_fpga_payload[pos++];
                }
            }
        ...
        else
        {
            ...
            Matrix flat_result_matrix = flattened_kernel * image_data_matrix; // Could probably reinterpret_cast this directly into a nxn Matrix, but I'm too lazy lol
            int pos = 0;
            for (int row = 0; row < result_matrix.get_num_rows(); row++)
            {
                for (int col = 0; col < result_matrix.get_num_cols(); col++)
                {
                    result_matrix[row][col] = flat_result_matrix[0][pos++];
                }
            }
        }
